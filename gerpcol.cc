...
string suffix = ".rates";
string refseq = "";
std::vector<std::string> refSeqVec;
bool proj_ref = false;
double trtv = 2.0;
...
void processMAF(ofstream &fout, ETree &src, double fq[]) {
  ...
  while (true) {
	...
	if (string("ACGTacgt").find(h[refseq][i]) == string::npos) {
            fout << "Pos:\t" << i << "\t" << "Spec:\t" << refseq << "\t" << "Letter ref:\t" <<  h[refseq][i] << "\t" << -1 << "\t" << -1 << endl;
	...
	fout << "Pos:\t" << i << "\t" << "Spec:\t" << refseq << "\t" << "Letter ref:\t" <<  h[refseq][i] << "\t" << rexp << '\t' << (rexp - robs) << endl;
      }
    }
  }
...
void processMFA(ofstream &fout, ETree &src) {
  ...
  fout << "Pos\tNeutralRate\tRS_score\t";
  for(int idx = 0; idx < refSeqVec.size(); idx++){
      if(idx == (refSeqVec.size() - 1)){
          fout << refSeqVec[idx] << "\n";
      } else {
          fout << refSeqVec[idx] << "\t";
      }
  ...
  for (unsigned int i = 0; i < L; i++) {
      ...
      bool go = true;
        for (int idx = 0; idx < refSeqVec.size(); idx++) {
            refseq = refSeqVec[idx];
            if(idx == 0){
                if (string("ACGTacgt").find(h[refseq]->getLetter(i + 1)) == string::npos) {
                    fout << -1 << "\t" << -1 << "\t" << h[refseq]->getLetter(i + 1) << "\t";
                    go = false;
                } else {
                    go = true;
                }
            } else if(idx == (refSeqVec.size() - 1)){
                if(!go){
                    fout << h[refseq]->getLetter(i + 1) << "\n";
                }
    ...
    //fout << i << "\t" << refseq << "\t" << h[refseq]->getLetter(i+1) << "\t" << rexp << '\t' << (rexp - robs) << endl;
    fout << rexp << '\t' << (rexp - robs) << "\t";
    for (int idx = 0; idx < refSeqVec.size(); idx++) {
            refseq = refSeqVec[idx];
            if(idx == (refSeqVec.size() - 1)){      
    ...
    //fout << "Pos:\t" << i << "\t" << "Spec:\t" << refseq << "\t" << "Letter ref:\t" << h[refseq]->getLetter(i+1) << "\t" << rexp << '\t' << (rexp - robs) << endl;
  }                     
}
...
//  }
//  
//  fout << "Pos\tSpec\tLetterRef\tV1\tV2"<< endl;
//  
//  for (unsigned int i = 0; i < L; i++) {
//    if (string("ACGTacgt").find(h[refseq]->getLetter(i+1)) == string::npos){
//        fout << i << "\t" << refseq << "\t" << h[refseq]->getLetter(i+1) << "\t" << -1 << "\t" << -1 << endl;
//        //fout << "Pos:\t" << i << "\t" << "Spec:\t" << refseq << "\t" << "Letter ref:\t" << h[refseq]->getLetter(i+1) << "\t" << -1 << "\t" << -1 << endl;
//      continue;
//    }
//    
//    if (proj_ref) h[refseq]->changeLetter('-', i+1);
...
int main (int argc, char *argv[]) {
  ...
  refSeqVec.clear();
  std::stringstream ss(refseq);
  std::string token;
  std::cout << refseq << std::endl;
  while(std::getline(ss, token, '-')){
      refSeqVec.push_back(token);
  }
  
  for(const auto & itt : refSeqVec){
      std::cout << itt << "\n";
  }
